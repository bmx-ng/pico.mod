...
for symbol in compare_case equals_case hash_case; do
	rg -q " bmx_embedded_string_${symbol}$" <<<"$compact_symbols"
done
if rg -q 'bmx_embedded_unicode_enable|bmx_pico_to_(lower|upper)_data|bmx_embedded_unicode_fold_character' <<<"$compact_symbols"; then
...
rg -q ' T bmx_embedded_unicode_enable$' <<<"$unicode_symbols"
rg -q ' r bmx_embedded_to_lower_data$' <<<"$unicode_symbols"
rg -q ' r bmx_embedded_to_upper_data$' <<<"$unicode_symbols"
rg -q ' T bmx_embedded_unicode_fold_character$' <<<"$unicode_symbols"
