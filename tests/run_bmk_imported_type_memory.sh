...
grep -q 'bmx_embedded_object_assert.*->_pico_tests_importedtypes_timportedcounter_value' "$application_c"
...
grep -q 'BMX_EMBEDDED_ARRAY_ELEMENT_VALUE, bbStructElementInit_pico_tests_importedtypes_TCounterMetadata, &bmx_pico_imported_value_' "$application_c"
grep -q 'BMX_EMBEDDED_VALUE_STRUCT, &bmx_pico_imported_value_.*TCounterMetadata' "$application_c"
...
producer_c="$(grep -l 'bmx_embedded_type_.*TImportedCounter_references' "$generated_dir"/module_*.c)"
test -n "$producer_c"
...
derived_c="$(grep -l 'bmx_embedded_type_.*TModuleCounter_references' "$generated_dir"/module_*.c)"
test -n "$derived_c"
