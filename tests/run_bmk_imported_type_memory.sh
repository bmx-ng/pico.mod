...
build_dir="$module_root/examples/.bmx/imported_type_memory.release.pico.arm.pico2"
generated_dir="$build_dir/generated"
bootstrap="$generated_dir/blitzmax_pico_modules.c"
test -s "$bootstrap"

module_sources=("$build_dir"/bcc/module_*/module.c)
test -s "${module_sources[0]}"
...
producer_unit_count="$(grep -l '^int __bb_pico_tests_importedtypes_' "${module_sources[@]}" | wc -l | tr -d ' ')"
test "$producer_unit_count" -eq 3

application_c="$build_dir/bcc/application/application.c"
grep -q '^struct pico_tests_importedtypes_TImportedCounter_obj {$' "$application_c"
grep -q 'BMXEmbeddedObject object;' "$application_c"
...
grep -q 'bmx_embedded_object_not_null.*->_pico_tests_importedtypes_timportedcounter_value' "$application_c"
...
grep -q '\.super = &bmx_embedded_imported_type_.*TModuleCounter' "$application_c"
grep -q 'TModuleCounter = .*\.super = &bmx_embedded_imported_type_.*TImportedCounter' "$application_c"
grep -q '(BMXEmbeddedMethod)_pico_tests_importedtypes_TImportedCounter_Current' "$application_c"
grep -q '(BMXEmbeddedMethod)_pico_tests_derivedtypes_TModuleCounter_Label' "$application_c"
grep -q 'static const BMXEmbeddedInterfaceEntry .*TLeafCounter_interfaces\[2\]' "$application_c"
grep -q 'bmx_embedded_interface_methods' "$application_c"
...
grep -q '_pico_tests_derivedtypes_TModuleCounter_EmbeddedBaseFinalize(object);' "$application_c"
...
grep -q 'BMX_EMBEDDED_ARRAY_ELEMENT_VALUE, bbStructElementInit_pico_tests_importedtypes_TCounterShape, 0' "$application_c"
grep -q 'BMX_EMBEDDED_ARRAY_ELEMENT_VALUE, bbStructElementInit_pico_tests_importedtypes_TCounterMetadata, &bmx_embedded_imported_value_' "$application_c"
grep -q 'BMX_EMBEDDED_VALUE_STRUCT, &bmx_embedded_imported_value_.*TCounterMetadata' "$application_c"
grep -q 'sizeof(struct pico_tests_importedtypes_TCounterMetadata), 2, BMX_EMBEDDED_VALUE_STRUCT' "$application_c"
grep -q '_fixedMetadata\[0\].*BMX_EMBEDDED_ROOT_STRUCT' "$application_c"
grep -q '_fixedMetadata\[1\].*BMX_EMBEDDED_ROOT_STRUCT' "$application_c"
...
producer_c="$(grep -l 'bmx_embedded_type_.*TImportedCounter_references' "${module_sources[@]}")"
test -n "$producer_c"
...
grep -q '^void _pico_tests_importedtypes_TImportedCounter_EmbeddedBaseInit' "$producer_c"
grep -q '^void _pico_tests_importedtypes_TImportedCounter_EmbeddedBaseFinalize' "$producer_c"
...
shared_c="$(grep -l 'bmx_global_.*sharedMetadata\[2\]' "${module_sources[@]}")"
test -n "$shared_c"
grep -q 'bmx_global_.*sharedMetadata\[0\].*BMX_EMBEDDED_ROOT_STRUCT' "$shared_c"
grep -q 'bmx_global_.*sharedMetadata\[1\].*BMX_EMBEDDED_ROOT_STRUCT' "$shared_c"

derived_c="$(grep -l 'bmx_embedded_type_.*TModuleCounter_references' "${module_sources[@]}")"
test -n "$derived_c"
...
grep -q '^void _pico_tests_derivedtypes_TModuleCounter_EmbeddedBaseInit' "$derived_c"
grep -q '^void _pico_tests_derivedtypes_TModuleCounter_EmbeddedBaseFinalize' "$derived_c"
