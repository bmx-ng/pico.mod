...
"$bmk" makeapp \
	...
	-o "$work_dir/closure_memory" \
	"$module_root/examples/closure_memory.bmx"

test -s "$work_dir/closure_memory.elf"
test -s "$work_dir/closure_memory.uf2"
...
read -r text_size _ bss_size _ < <("$toolchain/bin/arm-none-eabi-size" "$work_dir/closure_memory.elf" | awk 'NR == 2')
