...
If BLEInitialize("BlitzMax Test Peripheral") <> 0 Then Throw "BLE initialization failed"
If Not BLEWaitReady(10000) Then Throw "BLE controller did not become ready"
If BLEStartAdvertising(service, 0) <> 0 Then Throw "GATT advertising failed"
If Not BLEAdvertisingActive() Then Throw "GATT advertising inactive after start"
Print "Pico GATT peripheral ready"

Local subscribed:Int
Local receivedWrite:Int
While Not (subscribed And receivedWrite)
    Select PollEvent()
        ...
        Case EVENT_BLEGATTSUBSCRIBE
            ...
            Print "Notifications enabled: " + subscribed
        Case EVENT_BLEGATTWRITE
            Local writeEvent:TBLEGATTWriteEvent = TBLEGATTWriteEvent(EventExtra())
            If writeEvent.characteristic And writeEvent.value.length > 0
                receivedWrite = True
                Print "Received " + writeEvent.value.length + " bytes"
                If writeEvent.characteristic.SetValue(writeEvent.value, True) <> 0 Then ..
                    Throw "GATT notification failed"
            End If
        Case EVENT_BLERESET
            Throw "BLE controller reset: " + EventData()
    End Select
    Delay 1
Wend
Print "Pico GATT peripheral round trip passed"
...
Next
While BLEConnectionCount() And MilliSecs() < disconnectDeadline + 5000
    PollSystem()
    Delay 1
Wend
If BLEDeinitialize() <> 0 Then Throw "BLE teardown failed"
Print "Pico BLE deinitialized cleanly"
' Keep USB responsive for the next hardware test.
While True
    PollSystem()
    Delay 100
Wend
